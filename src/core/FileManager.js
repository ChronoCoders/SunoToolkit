...
class FileManager {
  ...
  async ensureDirectory(dirPath) {
    try {
      ...
      this.logger.error("Dizin oluşturma hatası", {
        path: dirPath,
        error: error.message,
      });
      throw error;
  ...
  async readFile(filePath, encoding = "utf8") {
    try {
      ...
      this.logger.error("Dosya okuma hatası", {
        path: filePath,
        error: error.message,
      });
      throw error;
  ...
  async writeFile(filePath, content, encoding = "utf8") {
    try {
      ...
      this.logger.error("Dosya yazma hatası", {
        path: filePath,
        error: error.message,
      });
      throw error;
  ...
  async deleteFile(filePath) {
    try {
      ...
      this.logger.error("Dosya silme hatası", {
        path: filePath,
        error: error.message,
      });
      throw error;
  ...
  async listFiles(dirPath, extension = null) {
    try {
      ...
      this.logger.debug(`Dosyalar listelendi: ${dirPath}`, {
        count: fullPaths.length,
      });
      return fullPaths;
    } catch (error) {
      this.logger.error("Dosya listeleme hatası", {
        path: dirPath,
        error: error.message,
      });
      throw error;
  ...
  async getFileSize(filePath) {
    try {
      ...
      this.logger.error("Dosya boyutu alma hatası", {
        path: filePath,
        error: error.message,
      });
      throw error;
  ...
  async writeJsonFile(filePath, data) {
    try {
      ...
      this.logger.error("JSON yazma hatası", {
        path: filePath,
        error: error.message,
      });
      throw error;
  ...
  async backupFile(filePath) {
    try {
      ...
      this.logger.info("Dosya backup alındı", {
        original: filePath,
        backup: backupPath,
      });
      return backupPath;
    } catch (error) {
      this.logger.error("Backup alma hatası", {
        path: filePath,
        error: error.message,
      });
      throw error;
  ...
  async cleanOldBackups(dirPath) {
    try {
      ...
      if (cleanedCount > 0) {
        this.logger.info("Eski backup dosyaları temizlendi", {
          count: cleanedCount,
        });
      }
      ...
      this.logger.error("Backup temizleme hatası", {
        path: dirPath,
        error: error.message,
      });
      throw error;
  ...
  async getDiskUsage(dirPath) {
    try {
      ...
      this.logger.error("Disk kullanımı hesaplama hatası", {
        path: dirPath,
        error: error.message,
      });
      throw error;
