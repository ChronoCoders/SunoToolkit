...
app.post("/api/generate", async (req, res) => {
  try {
    ...
    const { prompt, gpt_description_prompt, tags, make_instrumental, title } =
      req.body;
...
app.post("/api/batch-upload", upload.single("csvFile"), async (req, res) => {
  try {
    ...
    res.json({
      ...
      promptCount: prompts.length,
      estimatedTime:
        Math.ceil((prompts.length * (delay + 30000)) / 60000) + " dakika",
    });
  } catch (error) {
...
app.use((error, req, res, next) => {
  ...
  if (error instanceof multer.MulterError) {
    if (error.code === "LIMIT_FILE_SIZE") {
      return res
        .status(400)
        .json({ error: "Dosya boyutu cok buyuk (max 5MB)" });
    }
    return res
      .status(400)
      .json({ error: "Dosya yukleme hatasi: " + error.message });
  }
...
app.listen(PORT, HOST, () => {
  ...
  console.log(
    chalk.yellow(`📁 Static dosyalar: ${path.join(__dirname, "public")}`),
  );
  ...
  console.log(
    chalk.white("  GET  /api/songs           - Tum sarkilari listele"),
  );
